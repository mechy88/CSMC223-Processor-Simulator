   ...
   public:
      ...
      void start_process(Memory &memory) {
         ...
         for(InstructionCell cell : memory.mems1) {
            ...
            if(stoi(cell.address) < stoi(mar)) { continue; } //skips memory addresses when jmp opcode is executed
            ...
            if(opcode == "00") { //mov register to register movement
               ...
               memory.store(destination, formatString(intToHex(ac), 11));
      ...
      void printRegisters(Process &cpu){
         cout << "Register1 " << cpu.registers[0] << endl;
         cout << "Register2 " << cpu.registers[1] << endl;
         cout << "Register3 " << cpu.registers[2] << endl;
         cout << "Register4 " << cpu.registers[3] << endl;
         cout << "ac " << cpu.ac << endl;
